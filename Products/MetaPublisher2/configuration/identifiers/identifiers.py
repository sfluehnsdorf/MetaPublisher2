...
class Identifiers:
    ...
    security.declareProtected(
        permission_access_configuration, 'identifiers_form')

    identifiers_form = DTMLFile('identifiers', globals())

    security.declareProtected(
        permission_access_configuration, 'add_identifier_form')

    add_identifier_form = DTMLFile('add_identifier', globals())

    security.declareProtected(
        permission_access_configuration, 'delete_identifiers_form')

    delete_identifiers_form = DTMLFile('delete_identifiers', globals())

    security.declareProtected(
        permission_access_configuration, 'duplicate_identifiers_form')

    duplicate_identifiers_form = DTMLFile('duplicate_identifiers', globals())

    security.declareProtected(
        permission_access_configuration, 'edit_identifier_form')

    edit_identifier_form = DTMLFile('edit_identifier', globals())

    security.declareProtected(
        permission_access_configuration, 'rename_identifiers_form')
    ...
    def last_entry_id(self, identifier_id):
        """Return the last generated Entry id."""
        identifier = self.get_identifier(identifier_id)
        return identifier.last_entry_id()

    security.declareProtected(
        permission_access_configuration, 'last_entry_id_of_storage')

    def last_entry_id_of_storage(self, storage_id, identifier_id):
        """Return the last generated Entry id."""
    ...
    security.declareProtected(
        permission_access_configuration, 'new_entry_id_for_storage')

    def new_entry_id_for_storage(self, storage_id, identifier_id):
        """Generate a new Entry id, suitable for the specified Storage."""
        identifier = self.get_storage(storage_id).get_identifier(identifier_id)
        return identifier.new_entry_id()
...
# !!! identifiers.py - allow identifiers to be seperate from storage
#     (metapublisher2identifiers folder? or in storages folder)
