...
such as text, numbers, images, files, etc. Fields can be added, edited,
renamed, deleted and ordered as well as retrieved, listed and tested for
existence.
"""
...
class Fields:
    ...
    security.declareProtected(
        permission_access_configuration, 'add_field_form')

    add_field_form = DTMLFile('fields_add', globals())

    security.declareProtected(
        permission_access_configuration, 'delete_fields_form')

    delete_fields_form = DTMLFile('delete_fields', globals())

    security.declareProtected(
        permission_access_configuration, 'duplicate_fields_form')

    duplicate_fields_form = DTMLFile('duplicate_fields', globals())

    security.declareProtected(
        permission_access_configuration, 'change_field_type_form')

    change_field_type_form = DTMLFile('change_field_type', globals())

    security.declareProtected(
        permission_access_configuration, 'edit_field_form')

    edit_field_form = DTMLFile('edit_field', globals())

    security.declareProtected(
        permission_access_configuration, 'rename_fields_form')
    ...
    def add_field_type(self, storage_id, field_type, REQUEST=None):
        ...
        try:
            ...
            self.redirect(
                REQUEST,
                'fields_form',
                message='!TXT! Field type "%s" is invalid.' % (
                    REQUEST['storageType'])
            )
    ...
    def add_field(
        ...
        **args
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" of type "%s" added to Storage "%s".' % (
                field_id, field_type_id, storage_id)
        )
    ...
    def delete_field(self, storage_id, field_id, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" deleted.' % (
                field_id, storage_id)
        )
    ...
    def delete_fields(self, storage_id, field_ids=[], REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! %d Fields deleted from Storage "%s".' % (
                len(field_ids), storage_id)
        )
    ...
    def duplicate_field(self, storage_id, field_id, new_id, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" duplicated as "%s".' % (
                field_id, storage_id, new_id)
        )

    security.declareProtected(
        permission_change_configuration, 'duplicate_fields')
    ...
    def duplicate_fields(
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! %d Fields in Storage "%s" duplicated .' % (
                len(field_ids), storage_id)
        )
    ...
    def edit_field(
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" edited.' % (
                field_id, storage_id)
        )
    ...
    def rename_field(self, storage_id, field_id, new_id, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" renamed to "%s".' % (
                field_id, storage_id, new_id)
        )
    ...
    def rename_fields(self, storage_id, field_ids, new_ids, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! %d Fields in Storage "%s" renamed.' % (
                len(field_ids), storage_id)
        )
    ...
    def set_field_default(self, storage_id, field_id, value, REQUEST=None):
        ...
        self.redirect(
            ...
            'fields_form',
            message=(
                '!TXT! Default value of Field "%s" in Storage "%s" '
                'changed.' % (
                    field_id, storage_id))
        )
    ...
    security.declareProtected(
        permission_change_configuration, 'move_field_to_position')
    ...
    def move_field_to_position(
        ...
        self.redirect(
            ...
            'fields_form',
            message=(
                '!TXT! Field "%s" in Storage "%s" moved to position %d.' % (
                    field_id, storage_id, position))
        )
    ...
    def move_field_to_top(self, storage_id, field_id, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" moved to top.' % (
                field_id, storage_id)
        )
    ...
    def move_field_up(self, storage_id, field_id, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" moved up.' % (
                field_id, storage_id)
        )
    ...
    def move_field_down(self, storage_id, field_id, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" moved down.' % (
                field_id, storage_id)
        )
    ...
    def move_field_to_bottom(self, storage_id, field_id, REQUEST=None):
        ...
        self.redirect(
            REQUEST,
            'fields_form',
            message='!TXT! Field "%s" in Storage "%s" moved to bottom.' % (
                field_id, storage_id)
        )
