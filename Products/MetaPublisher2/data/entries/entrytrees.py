...
class EntryTrees:
    ...
    security.declareProtected(
        permission_access_entries, 'are_entries_branches')
    ...
    def is_ancestor(
        ...
        return source.is_ancestor(
            tree_field_id, parent_entry_id, entry_id, entry_position,
            other_parent_entry_id, other_entry_id, other_entry_position,
            of_level, max_levels)
    ...
    def count_ancestor_entries(
        ...
        return source.count_ancestor_entries(
            ...
            include_ancestor_siblings=include_ancestor_siblings)
    ...
    def move_entry_branch(
        ...
        self.redirect(
            REQUEST,
            'entries_form',
            message='!TXT! %d Entries in Storage "%s" moved.' % (
    ...
    def flatten_entry_branch(
        ...
        self.redirect(
            REQUEST,
            'entries_form',
            message='!TXT! %d Entries in Storage "%s" flattened.' % (
                len(entry_ids), source.getId())
        )
        return entry_ids, entry_positions
    ...
    def crop_entry_branch(
        ...
        self.redirect(
            REQUEST,
            'entries_form',
            message='!TXT! %d Entries in Storage "%s" cropped.' % (
                len(entry_ids), source.getId())
        )
        return entry_ids, entry_positions
