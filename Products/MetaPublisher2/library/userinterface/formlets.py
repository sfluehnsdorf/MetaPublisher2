...
class Formlets:
    ...
    def process_datatable_options(self, form_id, columns, rows, request):
        ...
        correct_form = (
            form.get('form_id', form_id) == form_id and true or false)

        # number of rows and size of batches
        rows_len = len(rows)
        default_batch_size = cookies.get(
            form_id + '_batch_size', self.get_setting('batches_default_size'))
        batch_size = int(
            correct_form and
            form.get('batch_size', default_batch_size) or
            default_batch_size)

        # first row to show (batch start)
        request_batch_start = (
            correct_form and
            form.get('batch_start', None) or
            None)
        if request_batch_start == 'first_batch':
            batch_start = 1
        elif request_batch_start == 'prev_batch':
            batch_start = max(
                1, int(cookies.get(form_id + '_batch_start', 1)) - batch_size)
        elif request_batch_start == 'next_batch':
            batch_start = min(
                rows_len,
                int(cookies.get(form_id + '_batch_start', 1)) + batch_size)
            ...
            batch_start = (
                correct_form and
                form.get('batch_start', default_batch_start) or
                default_batch_start)
        batch_start = (
            int((float(batch_start) - 1) / batch_size) * batch_size + 1)

        # sortable columns and which column to order by
        sortable_columns = filter(
            lambda column: column.get('sortable', None),
            columns)
        sortable_column_values = map(
            lambda column: column['value'],
            sortable_columns)
        if sortable_columns:
            default_order_by = (
                cookies.get(form_id + '_order_by', sortable_column_values[0]))
            order_by = (
                correct_form and
                form.get('order_by', default_order_by) or
                default_order_by)
        else:
            default_order_by = ''
            order_by = ''
        ...
        hidden_columns = filter(
            lambda column: column.get('hidden', None), hideable_columns)
        default_select_column_values = map(
            lambda column: column['value'],
            filter(
                lambda column: column.get('hidden', None) is false,
                hideable_columns))
        if correct_form and form_id in form:
            selected_columns = filter(
                lambda column: column['value'] in form.get('show_columns', []),
                hideable_columns)
        elif form_id + '_show_columns' in cookies:
            selected_columns = filter(
                lambda column: (
                    column['value'] in
                    cookies[form_id + '_show_columns'].split(',')),
                hideable_columns)
        else:
            selected_columns = filter(
                lambda column: column.get('hidden', None) is false,
        ...
        if correct_form and 'set_show_column' in form:
            ...
            selected_columns = filter(
                lambda column: (
                    column in selected_columns or
                    column.get('value', None) == value),
                hideable_columns)
        elif correct_form and 'unset_show_column' in form:
            value = form['unset_show_column']
            selected_columns = filter(
                lambda column: column.get('value', None) != value,
                selected_columns)
        selected_column_values = map(
            lambda column: column['value'], selected_columns)

        # columns to display and how
        revealed_columns = filter(
            lambda column: (
                column.get('hidden', None) is None or
                column in selected_columns),
            columns)
        revealed_column_values = map(
            lambda column: column['value'],
            revealed_columns)
        major_columns = filter(
            lambda column: not column.get('minor', None),
            revealed_columns)
        minor_columns = filter(
            lambda column: column.get('minor', None),
            revealed_columns)
        ...
        # !!! self.setProfileSetting(form_id + '_batch_size', batch_size,
        #     self.default_batch_size)
        # !!! self.setProfileSetting(form_id + '_show_columns',
        #     ','.join(selected_column_values),
        #     ','.join(default_select_column_values))
        # !!! self.setProfileSetting(form_id + '_order_by', order_by,
        #     default_order_by)
        assert default_select_column_values
        assert hidden_columns
        assert selected_column_values
    ...
    storage_selection_formlet = DTMLFile(
        'formlet_storage_selection', globals())
...
# !!! formlet.py - add table header & footer to be able to remove continued
#     flags
