...
class MetaTextWidget(WidgetPlugin):
    """MetaTextWidget"""

    meta_type = 'MetaTextWidget'

    # -------------------------------------------------------------------------

    pluginName = 'MetaTextWidget'
    ...
    _properties = WidgetPlugin._properties + (
        ...
        {'id': 'newline', 'type': 'boolean', 'mode': 'w'},
        {
    ...
    def setWidgetData(self, formTypeId, data={}):
        ...
        if data.get('pluginId__', '') == self.getWidgetId():
            ...
            self.newline = data.get('newline', None)
    ...
    def getWidgetData(self, formTypeId):
        ...
        return {
            ...
            'align': self.align,
            'valign': self.valign,
            'newline': self.newline and '1' or '0',
            'case': self.case,
            'quote': self.quote,
    ...
    def renderWidget(self, elementId, field, entryName=''):
        ...
        if formTypeId == 'List':
            ...
            if displayMode == 'defaultMode':
                result['field'] = (
                    '<textarea name="%s"' % fieldId + addInputOptions(self) +
                    '''><dtml-var "getZMP2().getField('%s', '%s').default">'''
            ...
            if displayMode == 'defaultMode':
                ...
                result['field'] = (
                    ...
                    addShowOptions(self) + '>')
                result['hidden'] = (
                    '<input type="hidden" name="%s" value="<dtml-var '
                    '''"getZMP2().getField('%s', '%s').default">">''' % (
    ...
    addFormlet = 'manage_MetaTextWidget_AddFormlet'
    editFormlet = 'manage_MetaTextWidget_EditFormlet'
    listFormlet = 'manage_MetaTextWidget_ListFormlet'
    viewFormlet = 'manage_MetaTextWidget_ViewFormlet'


# -----------------------------------------------------------------------------


manage_MetaTextWidget_AddFormlet = DTMLFile(
    'dtml/MetaTextWidget_AddFormlet', globals())
manage_MetaTextWidget_EditFormlet = DTMLFile(
    'dtml/MetaTextWidget_EditFormlet', globals())
manage_MetaTextWidget_ListFormlet = DTMLFile(
    'dtml/MetaTextWidget_ListFormlet', globals())
manage_MetaTextWidget_ViewFormlet = DTMLFile(
    'dtml/MetaTextWidget_ViewFormlet', globals())


# =============================================================================

def manage_addMetaTextWidget(self, id, formTypeId, data={}, REQUEST=None):
    "ZMI constructor for MetaTextWidget"

    instance = MetaTextWidget(id)
