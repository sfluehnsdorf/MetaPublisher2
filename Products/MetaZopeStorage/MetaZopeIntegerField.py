...
class MetaZopeIntegerField(FieldPlugin):
    "IntegerField"

    pluginName = 'IntegerField'
    pluginAuthor = 'Sebastian Luehnsdorf - Web-Solutions GbR.'
    pluginVersion = '1.0.2'
    pluginInfo = (
        'This an Integer Field for the MetaZopeStorage. Integers are whole '
        'numbers in the range of %d to %d.' % (-1 * (maxint + 1), maxint))
    ...
    _properties = FieldPlugin._properties + (
        {'id': 'default', 'type': 'int', 'mode': 'w'},
    )

    # -------------------------------------------------------------------------

    manage_configureFieldForm = DTMLFile(
        'dtml/editMetaZopeIntegerField', globals())
    ...
    def _setValue(self, entryId, value):
        ...
        if entry.hasProperty(fieldId):
            ...
            entry._setProperty(fieldId, value, 'int')
...
manage_addMetaZopeIntegerFieldForm = DTMLFile(
    'dtml/addMetaZopeIntegerField', globals())


def manage_addMetaZopeIntegerField(
    self, id, title='', default=0, REQUEST=None
):
    """ZMI constructor for MetaZopeIntegerField"""
    instance = MetaZopeIntegerField(id)
